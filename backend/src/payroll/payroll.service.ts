...
export class PayrollService {
  ...
  async updatePayroll(id: string, updatePayrollDto: UpdatePayrollDto): Promise<Payroll> {
  ...
  async bulkCreatePayroll(bulkPayrollDto: BulkPayrollDto): Promise<Payroll[]> {
    ...
    if (newPayrolls.length === 0) {
      throw new BadRequestException('All employees already have payroll records for this period');
    }
  ...
  async generatePayrollWithCalculations(period: string, employeeId?: string): Promise<any[]> {
    // Get employees
  ...
  async getPayrollSummary(period: string, employeeId?: string) {
    ...
    const currencyBreakdown = payrolls.reduce((acc: { [key: string]: number }, p) => {
